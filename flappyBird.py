...
def gameStart(bird, xyz):
    ...
    commentSuface = font.render('Click or use "ok" gesture to start', True, (0, 0, 0))
    ...
    while True:
        ...
        # Kiểm tra cử chỉ "ok" để bắt đầu game
        if xyz.gesture == "ok":
            return
...
def gamePlay(bird, columns, score, xyz):
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == MOUSEBUTTONDOWN:
                mouseClick = True
        if xyz.gesture == "palm":  # Kiểm tra nếu cử chỉ là "palm"
            mouseClick = True
        print(xyz.gesture)  # In cử chỉ để debug
        DISPLAYSURF.blit(BACKGROUND, (0, 0))
        columns.draw()
        columns.update()
        bird.draw()
...
def gameOver(bird, columns, score, xyz):
    font = pygame.font.SysFont('consolas', 60)
    headingSuface = font.render('GAMEOVER', True, (255, 0, 0))
    headingSize = headingSuface.get_size()
    
    font = pygame.font.SysFont('consolas', 20)
    commentSuface = font.render('Press "space" or use "ok" gesture to replay', True, (0, 0, 0))
    commentSize = commentSuface.get_size()
    ...
    while True:
        ...
        # Kiểm tra cử chỉ "ok" để reset game
        if xyz.gesture == "ok":
            return
...
def main():
    ...
    while True:
        ...
        gameOver(bird, columns, score, xyz)
